...
,(select count(distinct cc.contactscontactid)
from 
vansync_il_gov_2018.dnc_activityregions effective1 
join vansync_il_gov_2018.dnc_contactscontacts_myc cc on (cc.vanid = effective1.vanid and cc.committeeid = 60225
and date_trunc('week',(cc.datecanvassed::date)) = date_trunc('week',(current_date - interval '1 day')))
join vansync_il_gov_2018.contacttypecodes ctype on (ctype.contacttypeid = cc.contacttypeid and ctype.description ilike 'phone')
where effective1.foname = activity.foname
) 
RecruitmentCallsCompletedThisWeek
,(select count(distinct cc.contactscontactid)
from 
vansync_il_gov_2018.dnc_contactscontacts_myc cc 
join vansync_il_gov_2018.dnc_users users on cc.canvassedby = users.userid 
join jb4gov.fo_code_pod_turf_type orgcode on (orgcode.last ilike users.lastname and orgcode.first ilike users.firstname)
join vansync_il_gov_2018.contacttypecodes ctype on (ctype.contacttypeid = cc.contacttypeid and ctype.description ilike 'phone')
where cc.committeeid = 60225 
and cc.datecanvassed::date = (current_date - interval '1 day') and orgcode.org_code = activity.foname
) 
RecruitmentCallsbyFOyday
...
/*full outer join (select attempts.vanid 
			...
			join vansync_il_gov_2018.dnc_contacttypes ct on 
				...
				and attempts.datecanvassed::date >= date_trunc('week', (current_date - interval '1 day')) 
			left join vansync_il_gov_2018.dnc_users on 
...
select 
  collected_orgcode
  ,count(distinct case when biz_support ilike '%confirm%' and biz_action is not null then vanid else null end) biz
  ,coalesce((select minutes_logged from jb4gov.hubdialer_minutes_report hd where hd.org_code = ctv_biz_cb.collected_orgcode and hd.reporting_week =date_trunc('week',(current_date - interval '1 day'))::date ),0) dialer_minutes
  ,count(distinct case when date_trunc('week',ctv_date) = date_trunc('week',(current_date - interval '1 day')) then vanid else null end) ctv
  ,to_char(current_timestamp,'MM/DD HH:MIPM')
 -- ,count(distinct case when biz_support ilike '%confirm%' and biz_action = 'Display Polling Location Info' then vanid else null end) biz_reconfirm
  from jb4gov.CTV_Biz_CB
  group by collected_orgcode
  order by collected_orgcode;	
  
  select
ptb.org_code
,'filler' calls
,count(distinct pc.vanid_vf) people_knocked_tw
,to_char(current_timestamp,'MM/DD HH:MIPM')
from
jb4gov.person_corrected pc
join vansync_il_gov_2018.dnc_contactscontacts_vf ccvf on pc.vanid_vf = ccvf.vanid and ccvf.committeeid=60225 and ccvf.datecanvassed>=date_trunc('week',(current_date - interval '1 day'))
join vansync_il_gov_2018.dnc_contacttypes on ccvf.contacttypeid=dnc_contacttypes.contacttypeid and dnc_contacttypes.contacttypename = 'Walk'
join jb4gov.precinct_turfbook ptb on pc.precinctid = ptb.new_precinct_id and ptb.pgm_owner = 'Field - DVC'
group by ptb.org_code order by 1;
select 
collected_orgcode
,count(distinct case when biz_support ilike '%confirm%' and biz_action ilike '%early%vote%info%' then vanid else null end) biz_reconfirm
,to_char(current_timestamp,'MM/DD HH:MIPM')
  from jb4gov.CTV_Biz_CB
  group by collected_orgcode
  order by collected_orgcode;	
  select 
collected_orgcode
,count(distinct case when biz_support ilike '%confirm%' and biz_action = 'Early Vote Info' then vanid else null end) biz_reconfirm
,to_char(current_timestamp,'MM/DD HH:MIPM')
  from jb4gov.CTV_Biz_CB
  group by collected_orgcode
  order by collected_orgcode;	